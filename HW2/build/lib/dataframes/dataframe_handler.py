...
class DataFrameHandler:
    ...
    def update_with_new_data(self, collection_name):
        try:
            ...
            if data is not None and (collection_name == DatabaseCollections.onshape_logs.value or self.raw_df is None):
    ...
    def _dataframes_from_data(self, data, collection_source=DatabaseCollections.onshape_logs.value):
    ...
    def _populate_uploaded_logs(self):
        data = self.db_handler.read_from_database(DatabaseCollections.uploaded_logs.value)
        logs = []
        if data:
            for key in data:
            ...
            if self.raw_df is None:  # if the default path is not populated with data, populate it with the uploaded
                # logs
                self._dataframes_from_data(data, collection_source=DatabaseCollections.uploaded_logs.value)
    ...
    def _undo_redo_activity_detection(self):
        ...
        alerts = grouped[grouped['Count'] > os.environ['UNDO_REDO_THRESHOLD']]

        # Prepare the alerts DataFrame
        if not alerts.empty:
            alerts['Time'] = alerts['TimeWindow']
